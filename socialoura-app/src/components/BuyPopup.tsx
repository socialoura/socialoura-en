  ...
  return (
    <form onSubmit={handlePay} className="flex flex-col flex-1 min-h-0">
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
        {/* Order recap */}
        <div className="bg-[#F9FAFB] rounded-xl p-3.5 flex items-center justify-between border border-[#E5E7EB]">
          <div>
            <p className="text-sm font-bold text-[#111827]">{plan.productName}</p>
            <p className="text-xs text-[#6B7280]">
              {plan.quantity.toLocaleString()} {plan.type}
              {username ? ` · ${username}` : ""}
            </p>
          </div>
          <span className="text-lg font-extrabold text-[#111827]">${plan.price.toFixed(2)}</span>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              value={email}
              readOnly
              className="w-full h-12 px-4 rounded-xl border border-[#E5E7EB] text-[#111827] text-[15px] font-medium bg-[#F9FAFB] cursor-default"
            />
          </div>
        </div>
      ...
      {/* Sticky footer */}
      <div className="border-t border-[#E5E7EB] px-4 sm:px-6 py-4 bg-white shrink-0">
        <button
          type="submit"
    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute inset-0 flex items-end sm:items-center sm:justify-center sm:p-6 pointer-events-none">
        ...
        >
          ...
          {/* Header */}
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-[#F3F4F6] shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-[#FF4B6A]/10 flex items-center justify-center text-[#FF4B6A]">
                {platformIcon}
              </div>
              <div>
          ...
          {!done && (
            <div className="flex items-center gap-2 px-4 sm:px-6 py-3 shrink-0">
              <div className={`flex items-center gap-1.5 text-xs font-semibold ${step === "username" ? "text-[#FF4B6A]" : "text-[#9CA3AF]"}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${step === "username" ? "bg-[#FF4B6A] text-white" : "bg-[#E5E7EB] text-[#6B7280]"}`}>
                  {step === "payment" ? <CheckCircle className="w-3 h-3" /> : "1"}
                </div>
                Your info
              </div>
              <div className="flex-1 h-px bg-[#E5E7EB]" />
              <div className={`flex items-center gap-1.5 text-xs font-semibold ${step === "payment" ? "text-[#FF4B6A]" : "text-[#9CA3AF]"}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${step === "payment" ? "bg-[#FF4B6A] text-white" : "bg-[#E5E7EB] text-[#6B7280]"}`}>
                  2
                </div>
                Payment
              </div>
          ...
          {step === "username" && (
            <div className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-2 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      autoFocus
                      autoComplete="off"
                      className="w-full h-14 pl-12 pr-4 rounded-xl border border-[#E5E7EB] text-[#111827] text-[15px] font-medium bg-[#F9FAFB] focus:border-[#FF4B6A] focus:ring-2 focus:ring-[#FF4B6A]/10 outline-none transition-all"
                    />
                  </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#4B5563] mb-1.5">
                    Email address * <span className="font-normal text-[#9CA3AF]">(for order confirmation)</span>
                  </label>
                  <input
                    type="email"
                    value={email}
                    ...
                    autoComplete="email"
                    className={`w-full h-14 px-4 rounded-xl border text-[#111827] text-[15px] font-medium bg-[#F9FAFB] outline-none transition-all ${
